...
function startOfWeek(d: Date) {
  ...
  const dow = x.getDay(); // 0..6 (Sun start)
...
function ProgressBar({
  ...
  return (
    <div style={{ marginTop: 9 }}>
      ...
      {/* ✅ intentionally NO percentage text */}
    </div>
  );
}

function CenterBigPlus({ onClick }: { onClick: () => void }) {
  return (
...
export default function TasksPage() {
  ...
  const progressByTask = useMemo(() => {
    const map: Record<string, { done: number; target: number; ratio: number }> = {};
    const now = new Date();

    for (const t of tasks) {
      const target = t.type === "habit" ? Math.max(1, Number(t.freq_times ?? 1)) : 1;
      const freq: FrequencyUnit = t.type === "habit" ? (t.freq_per ?? "week") : "day";
      const periodStartISO = getPeriodStart(freq, now).toISOString();
      ...
      map[t.id] = { done, target, ratio: clamp01(done / target) };
    }
  ...
  function toDraftFromRow(r: ReminderRow): ReminderDraft {
    ...
    if (!days || days.length === 0) {
      return { enabled: !!r.enabled, timezone: tz, time_of_day: timeHHMM, cadence: "daily", days_of_week: [1] };
    }

    return { enabled: !!r.enabled, timezone: tz, time_of_day: timeHHMM, cadence: "weekly", days_of_week: [days[0]] };
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {hasOne ? (
          <div style={{ fontSize: 12, opacity: 0.75 }}>
            Note: Right now you can have <b>one</b> reminder per task.
          </div>
        ) : (
          <div style={{ opacity: 0.75, fontSize: 13 }}>No reminders set.</div>
        )}
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    position: "relative",
                    overflow: "hidden",
                    padding: "13px 12px 14px", // ✅ slightly taller (~5%)
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {/* small progress count like home cards */}
                  ...
                  {/* ✅ Progress bar, no % underneath */}
                  <ProgressBar ratio={p.ratio} bgIsDark={!textIsBlack} />
                </div>
              );
            })}
          </div>
        ...
        {/* ✅ Big plus centered under the bottom-most task list */}
        <CenterBigPlus onClick={() => router.push("/tasks/create")} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {archivedTasks.map((t) => (
                ...
                >
                  ...
                  <div style={{ opacity: 0.75, marginTop: 6, fontSize: 13 }}>Tap to edit</div>
                </button>
              ))}
            </div>
          </div>
